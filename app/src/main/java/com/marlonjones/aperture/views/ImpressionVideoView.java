...
public class ImpressionVideoView extends VideoView {
    ...
    public void pause(boolean fade) {
        ...
        super.pause();
        mIcon.setImageResource(com.marlonjones.aperture.R.drawable.ic_play);
        reset(mIcon);
        reset(mSeekerFrame);
        if (fade) {
            fade(mIcon, true, 0f);
    ...
    public void start() {
        ...
        super.start();
        startSeekerUpdates();
        mIcon.setImageResource(com.marlonjones.aperture.R.drawable.ic_pause);
        reset(mIcon);
        reset(mSeekerFrame);
        fade(mIcon, true, 0f);
        fade(mSeekerFrame, true, 0f);
    ...
    public void hookViews(ViewerPageFragment fragment, View overlay) {
        mFragment = fragment;
        mOverlay = overlay;
        mIcon = (ImageView) overlay.findViewById(com.marlonjones.aperture.R.id.icon);
        mPosition = (TextView) overlay.findViewById(com.marlonjones.aperture.R.id.position);
        mDuration = (TextView) overlay.findViewById(com.marlonjones.aperture.R.id.duration);
        mSeeker = (SeekBar) overlay.findViewById(com.marlonjones.aperture.R.id.seeker);
        mSeekerFrame = overlay.findViewById(com.marlonjones.aperture.R.id.seekerFrame);

        overlay.setBackgroundResource(Utils.resolveDrawable(fragment.getActivity(), com.marlonjones.aperture.R.attr.video_overlay_selector));
